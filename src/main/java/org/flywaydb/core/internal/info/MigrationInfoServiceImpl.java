...
import com.qpaas.migration.extend.undo.UndoContext;
...
public class MigrationInfoServiceImpl implements MigrationInfoService, OperationResult {
     ...
     * 执行回滚的信息
     */
    private List<MigrationInfoImpl> undoInfos;
    /**
     ...
     * @param cherryPick
     *            The migrations to consider when migration.
     * @param pending
     ...
     */
    public MigrationInfoServiceImpl(MigrationResolver migrationResolver, SchemaHistory schemaHistory, Database database,
        final Configuration configuration, MigrationVersion target, boolean outOfOrder, MigrationPattern[] cherryPick,
    ...
    public void refresh() {
        ...
        Map<Pair<MigrationVersion, Boolean>, ResolvedMigration> undoVersioned = new TreeMap<>();
        ...
        for (Pair<AppliedMigration, AppliedMigrationAttributes> av : appliedVersioned) {
            ResolvedMigration resolvedMigration =
                resolvedVersioned.get(Pair.of(av.getLeft().getVersion(), av.getLeft().getType().isUndo()));
            ...
            migrationInfos1.add(new MigrationInfoImpl(resolvedMigration, av.getLeft(), context,
                av.getRight().outOfOrder, av.getRight().deleted, av.getRight().undone));
        }
        ...
        if (configuration.getFailOnMissingTarget() && target != null && target != MigrationVersion.CURRENT
            && target != MigrationVersion.LATEST && target != MigrationVersion.NEXT) {
            ...
            if (!targetFound) {
                throw new FlywayException("No migration with a target version " + target
        ...
        for (Pair<AppliedMigration, AppliedMigrationAttributes> av : appliedRepeatable) {
            ...
            if (!av.getRight().deleted && av.getLeft().getType() != MigrationType.DELETE && resolvedMigration != null
                && rank == latestRank && resolvedMigration.checksumMatches(appliedRepeatableMigration.getChecksum())) {
        ...
        // undo 数据集的赋值
        // Add pending repeatables to output list
        List<MigrationInfoImpl> undoList = new ArrayList<>();

        for (ResolvedMigration resolvedMigration : resolvedMigrations) {
            undoList.add(new MigrationInfoImpl(resolvedMigration, null, context, false, false, false));
        }
        undoInfos = undoList;
     ...
     * @param appliedRepeatable
     *            The discovered applied migrations
     */
    ...
    public MigrationInfo current() {
        ...
        for (MigrationInfoImpl migrationInfo : migrationInfos) {
            if (migrationInfo.getState().isApplied() && !MigrationState.DELETED.equals(migrationInfo.getState())
                && !migrationInfo.getType().equals(MigrationType.DELETE) && migrationInfo.getVersion() != null
    ...
    public MigrationInfoImpl[] undo() {
        String undoPath = UndoContext.getUndoContext().getUndoPath();
        List<MigrationInfoImpl> pendingMigrations = new ArrayList<>();
        boolean check = check(undoPath);
        if (!check) {
            throw new FlywayException("script[" + undoPath + "] is invalid ,it has no V version");
        }
        for (MigrationInfoImpl migrationInfo : undoInfos) {
            if (MigrationType.UNDO_SQL == migrationInfo.getType()
                || MigrationType.UNDO_JDBC == migrationInfo.getType()) {
                if (StringUtils.isNotBlank(undoPath)) {
                    String comparePath = "";
                    boolean match = false;
                    if (migrationInfo.getResolvedMigration().getType() == MigrationType.UNDO_JDBC) {
                        match =
                            migrationInfo.getResolvedMigration().getScript().endsWith(undoPath.replaceAll("/", "."));
                    } else {
                        match = migrationInfo.getResolvedMigration().getScript().equals(undoPath);
                    }
                    if (match) {
                        pendingMigrations.add(migrationInfo);
                    }
        ...
        if (pendingMigrations.size() == 0) {
            System.err.println("path [" + undoPath + "] is invalid ,it is not contains in  Undo list");

        }
        return pendingMigrations.toArray(new MigrationInfoImpl[0]);
    }

    private boolean check(String undoPath) {
        String undoVersion = undoPath.substring(undoPath.indexOf("U") + 1, undoPath.indexOf("__"));
        if(StringUtils.isNotBlank(undoVersion)){
            undoVersion = undoVersion.replace("_", ".");
        }
        for (MigrationInfoImpl migrationInfo : migrationInfos) {
            AppliedMigration appliedMigration = migrationInfo.getAppliedMigration();
            if (Objects.isNull(appliedMigration) || Objects.isNull(appliedMigration.getVersion())) {
                continue;
            }

            if (appliedMigration.getVersion().getVersion().equals(undoVersion)) {
                return true;
            }
    ...
    public InfoResult getInfoResult(MigrationInfo[] infos) {
        return CommandResultFactory.createInfoResult(this.context.getConfiguration(), this.database, infos,
            this.current(), this.allSchemasEmpty);
    }
