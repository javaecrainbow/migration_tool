...
import com.qpaas.migration.extend.undo.DbUndo;
...
import org.flywaydb.core.internal.resolver.CompositeUndoMigrationResolver;
...
public class Flyway {
    ...
    public UndoResult undo(String scriptPath) throws FlywayException {
        // 设置线程上线文
        //扩展undo功能
        UndoContext.UndoParam undoParam = new UndoContext.UndoParam().setUndo(true);
        if(org.apache.commons.lang.StringUtils.isNotBlank(scriptPath)){
            undoParam.setUndoPath(scriptPath);
        }
        UndoContext.setUndoContext(undoParam);
        return execute(new Command<UndoResult>() {
            @Override
            public UndoResult execute(MigrationResolver migrationResolver, SchemaHistory schemaHistory, Database database, Schema[] schemas, CallbackExecutor callbackExecutor, StatementInterceptor statementInterceptor) {
                UndoResult result = new DbUndo(database, schemaHistory, schemas[0], migrationResolver, configuration, callbackExecutor).migrate();
                //callbackExecutor.onOperationFinishEvent(Event.AFTER_UNDO_OPERATION_FINISH, result);
                return result;
            }
        }, true);
    }
     ...
     * @return A new, fully configured, MigrationResolver instance.
     */
    private MigrationResolver createUndoMigrationResolver(ResourceProvider resourceProvider,
                                                      ClassProvider<JavaMigration> classProvider,
                                                      SqlScriptExecutorFactory sqlScriptExecutorFactory,
                                                      SqlScriptFactory sqlScriptFactory,
                                                      ParsingContext parsingContext) {
        return new CompositeUndoMigrationResolver(resourceProvider, classProvider, configuration,
                sqlScriptExecutorFactory, sqlScriptFactory, parsingContext, configuration.getResolvers());
    }
    ...
    /*private -> testing*/ <T> T execute(Command<T> command, boolean scannerRequired) {
        ...
        try {
            ...
            if(UndoContext.getUndoContext().isUndo()){
                result = command.execute(
                        createUndoMigrationResolver(resourceProvider, classProvider, sqlScriptExecutorFactory, sqlScriptFactory, parsingContext),
                        schemaHistory,
                        database,
    ...
    /*private -> testing*/ interface Command<T> {
         ...
         */
        T execute(MigrationResolver migrationResolver, SchemaHistory schemaHistory, Database database, Schema[] schemas, CallbackExecutor callbackExecutor, StatementInterceptor statementInterceptor);
    }
}