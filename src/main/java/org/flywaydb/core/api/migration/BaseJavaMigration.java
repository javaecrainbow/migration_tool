 ...
 * state scripts</li>
 * <li><strong>Version:</strong> Underscores (automatically replaced by dots at runtime) separate as many parts as you
 * like (Not for repeatable migrations)</li>
...
public abstract class BaseJavaMigration implements JavaMigration {
    ...
    public BaseJavaMigration() {
        ...
        if (shortName.startsWith("V") || shortName.startsWith("U") || repeatable) {
            prefix = shortName.substring(0, 1);
        }
        if (prefix == null) {
            throw new FlywayException(
                "Invalid Java-based migration class name: " + getClass().getName() + " => ensure it starts with V, R" +
